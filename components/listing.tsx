import { Box, Flex, Link, Heading, Text, Spacer, VStack, Skeleton, textDecoration, color } from "@chakra-ui/react"
...
const List = () => {
    ...
    if(!data)
    return (
        <Box as="section" bg="white" border="1px solid #E2E4E6" width="100%">
            <ListHeading/>
            {[1, 2, 3, 4, 5].map(id => {
                return (
...
const ListBox = ({ title, category, slug }) => {
    return (
        <Link href={`https://dev.to/listings/${category}/${slug}`} _hover={{textDecoration: "none", color: "blue"}} isExternal>
    ...
    return (
        <Box as="section" bg="white" borderRadius="md" border="1px solid #E2E4E6">
            ...
            {[
                ...
                {
                    title: 'Pair Programming with Jhey', 
                    category: "events", 
                    slug: ""
                },
            ].map((news, index) => (
